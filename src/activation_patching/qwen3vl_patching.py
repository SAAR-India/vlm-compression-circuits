"""
Qwen3-VL activation patching (NOTICE / Edge Activation Patching style).

Qwen3-VL uses an LLM-based architecture (model.model.language_model.layers) with self-attention and MLP.
Vision is merged into the sequence via model.visual.merger; there is no separate cross-attention
in the LLM (unlike BLIP's Q-Former). Patching follows NOTICE methodology:
...
def _qwen_inputs_for_forward(processor, image, prompt_text: str, device, batch_size: int = 1):
    """Build model inputs using apply_chat_template(tokenize=True)."""
    ...
    if batch_size == 1:
        ...
        # Batch: process each (identical) and stack
        batch_inputs = []
...
def _forward_logits(model, processor, image, prompt_text: str, use_amp: bool = False):
    ...
    inputs = _qwen_inputs_for_forward(processor, image, prompt_text, device, batch_size=1)
    amp_ctx = (
...
def _forward_logits_batched(
    ...
    device = _get_device(model)
    inputs = _qwen_inputs_for_forward(processor, image, prompt_text, device, batch_size=batch_size)
    amp_ctx = (
        torch.amp.autocast("cuda", dtype=torch.float16)
        if (use_amp and device.type == "cuda")
        else nullcontext()
...
def _map_tokens_to_labels_qwen(
    processor, prompt_text: str, correct_answer, incorrect_answer, num_tokens: int
) -> List[str]:
    """Map token positions to semantic labels for Qwen tokenizer.
    num_tokens comes from activation shape; prompt text is tokenized to infer label positions.
    For Qwen, image tokens are interleaved so we pad labels at the start (image tokens = 'other').
    """
    ...
    # Align to activation sequence length: pad with "other" at start (image tokens), truncate if needed
    text_len = len(labels)
...
def _aggregate_by_semantic_group(
    ...
    """Aggregate per-sample scores by token label. Qwen has no cross-attn, so xattn=None."""
...
def _get_llm_layers(model):
    """Get LLM decoder layers (model.model.language_model.layers)."""
...
def compute_qwen3vl_patching_scores(
    model,
    processor,
    dataset: List[Dict],
    num_samples: int,
    ...
    """
    Compute per-layer, per-token, per-component patching scores for Qwen3-VL.
    Returns (avg_mlp, avg_attn, xattn=None, token_labels, all_sample_results).
    Qwen3-VL has no cross-attention in LLM; vision is merged into the sequence.
    """
    ...
    for sample_idx in tqdm(range(n), desc="Patching samples", unit="sample"):
        ...
        token_labels = _map_tokens_to_labels_qwen(
        ...
        # Qwen uses pre-norm; residual is added after sublayer. Patch the sublayer output directly.
        def make_qwen_patch_hook(contribution_clean, token_indices):
            ...
            for lid in range(num_layers):
                for comp, scores, get_target in [
                    ...
                    for tok_start in range(0, num_tokens, patch_batch_size):
                        ...
                        h = target.register_forward_hook(make_qwen_patch_hook(src, tok_batch))
...
def run_qwen3vl_patching(
    ...
    """
    Run activation patching for Qwen3-VL (uncompressed or compressed).
    ...
    out_path = output_dir / f"qwen3vl__{model_variant}"
    ...
    if model_variant == "baseline":
        model, processor = load_uncompressed_model("qwen3vl")
        model_label = "Qwen3-VL Uncompressed"
    else:
        model, processor = load_compressed_model("qwen3vl", model_variant, "V_P")
        model_label = f"Qwen3-VL {model_variant.upper()}"

    model = model.to(device)
    model.eval()

    mlp, attn, xattn, tok_labels, all_results = compute_qwen3vl_patching_scores(
    ...
    if mlp is None:
        return {"model": "qwen3vl", "variant": model_variant, "error": "No successful samples"}
